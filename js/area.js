...
class App {
    ...
    tabs() {
        const $wrap = $('.wrap-tabs');
        function switchTab(tabIndex) {
            $wrap.find('.wrap-tabs__btn').removeClass('active');
            $wrap.find('.wrap-tabs__content-el').hide();
            $wrap.find('.wrap-tabs__btn').eq(tabIndex).addClass('active');
            $wrap.find('.wrap-tabs__content-el').eq(tabIndex).fadeIn(400);
        }

        switchTab(0);

        $wrap.on('click', '.wrap-tabs__btn', function() {
    ...
    form() {
        ...
        function checkValid($inp) {
            ...
            if (val === '' || val === undefined || val == null) {
                ...
                $wrap.find('.form-el__message').fadeOut(200);
                if ($inp.hasClass('inp-email')) {
                    if (isEmail(val)) {
                        $wrap.removeClass('error');
                        return true;
                    } else {
        ...
        $('.form-def').on('submit', function(e) {
            e.preventDefault();
            let $form = $(this);
            let countR = 0;
            let countEls = $form.find('.form-def__inp').length;
            let formData = new FormData();
            $form.find('.form-def__inp').each(function() {
                let $inp = $(this);
                if (checkValid($inp)) {
                    countR++;
            ...
            if (countEls === countR) {
                $form.find('.block-form__error').hide();
                $form.find('.block-form__inp').removeClass('active');
                $form.find('.block-form__content').addClass('hide');
                $form.find('.block-form__success-wrap').fadeIn(300);
                $form.find('.redirectpay').click();
